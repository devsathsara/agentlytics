  ...
  return (
    <div className="fade-in space-y-6">
      <PageHeader icon={LayoutDashboard} title="Dashboard">
        <div className="ml-auto flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1 text-[12px] transition hover:opacity-80"
        ...
        <div className="flex items-center flex-wrap gap-1.5">
          {allEditors.map(e => {
            ...
            return (
              <button
                key={e.id}
                className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-[12px] cursor-pointer transition"
                style={{
      ...
      <section className="space-y-3">
        <div className="text-[10px] uppercase tracking-wider font-medium" style={{ color: 'var(--c-text3)' }}>Snapshot</div>
        ...
        <div className="card p-3">
          ...
          <div className="flex gap-4 flex-wrap lg:flex-nowrap">
            ...
            {stats && dailyData && (() => {
              const activeDays = dailyData.filter(day => day.total > 0)
              const busiest = activeDays.length > 0 ? activeDays.reduce((a, b) => a.total > b.total ? a : b) : null
              const totalSessions = activeDays.reduce((s, day) => s + day.total, 0)
      ...
      {/* ── 2. Patterns ── */}
      <section className="space-y-3">
        <div className="text-[10px] uppercase tracking-wider font-medium" style={{ color: 'var(--c-text3)' }}>Patterns</div>
      ...
      {/* ── 3. Breakdown ── */}
      <section className="space-y-3">
        <div className="text-[10px] uppercase tracking-wider font-medium" style={{ color: 'var(--c-text3)' }}>Breakdown</div>
            ...
            <div className="space-y-1 max-h-[180px] overflow-y-auto scrollbar-thin">
              {d.topProjects.slice(0, 12).map(p => (
                <div key={p.name} className="flex items-center gap-1.5">
                  ...
                  <div className="flex-1 h-3 overflow-hidden" style={{ background: 'var(--c-code-bg)' }}>
                    <div className="h-full bg-accent/30" style={{ width: `${(p.count / maxProject * 100).toFixed(1)}%` }} />
                  </div>
                ...
                {stats.topModels.map((m, i) => {
                  ...
                  return (
                    <div key={m.name} className="flex items-center gap-2">
                      ...
                      <div className="flex-1 h-4 overflow-hidden" style={{ background: 'var(--c-code-bg)' }}>
                        <div className="h-full" style={{ width: `${(m.count / maxM * 100).toFixed(1)}%`, background: i === 0 ? '#6366f1' : i === 1 ? '#818cf8' : '#a5b4fc40' }} />
                      </div>
                ...
                {stats.topTools.map((t, i) => {
                  ...
                  return (
                    <div key={t.name} className="flex items-center gap-2">
                      <span className="text-[10px] w-3 text-right" style={{ color: 'var(--c-text3)' }}>{i + 1}</span>
                      <div className="flex-1 h-4 overflow-hidden" style={{ background: 'var(--c-code-bg)' }}>
                        <div className="h-full flex items-center px-1.5" style={{ width: `${(t.count / maxT * 100).toFixed(1)}%`, background: i === 0 ? '#10b981' : i === 1 ? '#34d399' : '#6ee7b740' }}>
            ...
            <div className="card p-3">
              ...
              <div className="space-y-1">
                {largeContextChats.slice(0, 10).map(c => (
                  <div
                    key={c.id}
                    className="flex items-center gap-1.5 px-1.5 py-1 cursor-pointer transition hover:opacity-80"
